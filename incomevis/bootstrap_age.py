...
def uni_sample(raw, n=100, year_start=1977, year_end=2019):
...
def age_sample_incomevis(dist_age, year_start=1977, year_end=2019, incomeType='RPPERHHINCOME', k = 'decile',
                         subpop = False, black = None, hispan = None, sex = None, educ = None,
                         output_path='gdrive/My Drive/Colab Notebooks/incomevis/analysis/bootstrap/withreplacement/bootstrap_age/data/'):
    sub_raw = dist_age
    if subpop:
    ...
    statefip = [1, 2, 4, 5, 6, 8, 9, 10, 11, 12, 13, 15, 16, 17, 18, 19, 20,
    ...
    if k == 'decile':
        ...
        result = pd.DataFrame(index = decile_column, columns=statefip)

        # Iterate through each state
        c = 0
        for state in statefip:
        ...
        result = pd.merge(result, pop['UR_NORMPOP_' + str(year)], left_index = True, right_index = True)
    ...
    if k == 'percentile':
        ...
        result = pd.DataFrame(index = percentile_column, columns=statefip)
